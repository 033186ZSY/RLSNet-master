...
class RLSNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        语义分割分支
        """
        x22 = self.seg_header2(x2) # 输出 x2 = Size([1, 128, 90, 160])
        x33 = self.seg_header3(x3) 
        x33 = nn.functional.interpolate(x33, size=(90, 160), mode='bilinear',align_corners=False) #输出 x3 = Size([1, 128, 90, 160])
        x44 = self.seg_header4(fea)
        x44 = nn.functional.interpolate(x44, size=(90, 160), mode='bilinear', align_corners=False) #输出 x4 = Size([1, 128, 90, 160])
        seg = torch.cat([x22, x33, x44],dim=1) #输出 seg = Size([1, 128, 90, 160])
        ...
        if self.training == True:
            return out_drivable, out_lane, output_scenes, x2, x3, fea
        else:
