...
from rlsnet.model_v4 import RLSNet
import os
import torch
import tqdm
import numpy as np
...
def SAD_loss(layer1, layer2, scale = 2):
    ...
    map1 = map1.view(shape[0],shape[1],shape[2])

    layer2.detach()

    map2 = torch.pow(layer2,2).sum(1,keepdim=True)
    map2 = F.interpolate(map2, size = (shape[1],shape[2]), mode="bilinear")
    map2 = map2.squeeze(1)
    map2 = map2.view(shape[0],-1)
    map2 = F.softmax(map2,dim=1)
    map2 = map2.view(shape[0],shape[1],shape[2])
...
def train(args, model, optimizer, dataloader_train, dataloader_val):
    ...
    for epoch in range(args.num_epochs):
        ...
        for sample, scenes_id in dataloader_train:
            ...
            out_drivable, out_lane, output_scenes, x2, x3, fea = model(data)
            ...
            if epoch > 30:
                loss_sad1 = SAD_loss(x2, x3)
                loss_sad2 = SAD_loss(x3, fea)
                loss += 0.1*(loss_sad1 + loss_sad2)
        ...
        if epoch % args.validation_step == 0:
            ...
            Epoch_result.write(str(epoch)+', {:.6f}, {:.6f}, {:.6f}, {:.6f}, {:.6f}, {:.6f}, {:.6f}, {:.6f}, {:.6f}, {:.6f}'.format(
                Acc_pix_drivable, Acc_class_drivable, mIoU_drivable, FWIoU_drivable,\
                Acc_pix_lane,     Acc_class_lane,     mIoU_lane,     FWIoU_lane, \
...
def make_data_loader(args, **kwargs):
    ...
    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, sampler=train_sampler, **kwargs)
...
if __name__ == '__main__':
    params = [
        ...
        '--save_model_path', '/workspace/xiaozhihao/RLSnet/checkpoint_of_XZH/(2022.10.16)RLS_resnet50_sad_ddp(v4)/',
