"""
RLSNet在bdd100k数据集上训练, 此代码为论文中的消融实验
"""
...
from util.loss import OhemCrossEntropy
from utils import create_logger
from dataset import bdd100k_v2, base_dataset, bdd100k
import pprint
...
def val(args, epoch, model, dataloader):
    ...
    for i, (sample, scenes_id) in enumerate(tbar):
        ...
        elif args.drivable_only:
            images, drivable_targets = sample['image'], sample['drivable_label']
            if args.use_gpu:
                images, drivable_targets, scenes_label = images.cuda(), sample['drivable_label'].cuda(), scenes_id.cuda()
        elif args.lane_only:
            images, lane_targets = sample['image'], sample['lane_label']
            if args.use_gpu:
                images, drivable_targets, scenes_label = images.cuda(), sample['lane_label'].cuda(), scenes_id.cuda()
...
def train(args, model, optimizer, dataloader_train, dataloader_val, logger, final_output_dir):
    """模型训练"""
    lane_loss = drivable_loss = OhemCrossEntropy(ignore_label = args.ignore_label).cuda()
    ...
    for epoch in range(args.num_epochs):
        ...
        for sample, scenes_id in dataloader_train:
            if args.whole:
                ...
                data, drivable_label = sample['image'], sample['drivable_label']#torch.Size([1, 720, 1280]) #torch.Size([1])
                if torch.cuda.is_available() and args.use_gpu:
                    data = data.cuda()
                    drivable_label = drivable_label.cuda()
                    scenes_label = scenes_id.cuda()
                ...
                """计算各项任务的损失"""
                loss_drivable = drivable_loss(out_drivable, drivable_label.long())
                loss_lane ,loss_scenes = 0, 0
                loss = loss_drivable
            
            elif args.lane_only:
                data, lane_label = sample['image'], sample['lane_label']#torch.Size([1, 720, 1280]) #torch.Size([1])
                if torch.cuda.is_available() and args.use_gpu:
                    data = data.cuda()
                    lane_label = lane_label.cuda()
                ...
                """计算各项任务的损失"""
                loss_lane = lane_loss(out_lane, lane_label.long())
                loss_drivable, loss_scenes = 0, 0
                loss = loss_lane

            elif args.scenes_only:
                data = sample['image']#torch.Size([1, 720, 1280]) #torch.Size([1])
                if torch.cuda.is_available() and args.use_gpu:
                    data = data.cuda()
                    scenes_label = scenes_id.cuda()
        ...
        if epoch % args.checkpoint_step == 0 :
            ...
            torch.save(model.module.state_dict(), os.path.join(final_output_dir, str(epoch)+'.pth'))
...
def main(params):
    ...
    gpus = [0,1]
    if torch.cuda.is_available() and len(gpus)<2:
        os.environ['CUDA_VISIBLE_DEVICES'] = str(gpus[0])
        model = torch.nn.DataParallel(model).cuda()

    elif torch.cuda.is_available() and len(gpus)==2:
        device=torch.device('cuda:{}'.format(gpus[0])) # 设置0号GPU是主GPU
    ...
    # 5、开始训练
    train(args, model, optimizer, dataloader_train, dataloader_val, logger, final_output_dir)
...
if __name__ == '__main__':
    params = [
        ...
        '--scenes_classes', '4',
        '--cuda', '1', 
        '--batch_size', '6',  # 6 for resnet101, 20 for resnet18
        '--save_model_path', '/workspace/xiaozhihao/RLSnet/checkpoint_of_XZH/(2022.12.17)Ablation/',
